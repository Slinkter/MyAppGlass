...
describe('Formulario de Contacto - contactFormSchema Zod Validation', () => {
  ...
  it('❌ debe fallar si el nombre tiene menos de 2 caracteres', () => {
    const result = contactFormSchema.safeParse({ ...validContact, name: 'C' });
  ...
  it('❌ debe fallar si el nombre excede 100 caracteres', () => {
    ...
    const result = contactFormSchema.safeParse({ ...validContact, name: longName });
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it('❌ debe fallar con teléfonos no válidos (letras o muy cortos no vacíos)', () => {
  ...
  it('✅ debe permitir teléfono vacío u opcional', () => {
    const resultWithoutPhone = contactFormSchema.safeParse({
      name: 'Carlos Mendoza',
      email: 'carlos.mendoza@example.com',
      message: 'Hola, deseo solicitar una cotización formal.',
    });
