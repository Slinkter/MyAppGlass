 ...
 */
const normalizeYear = (year: string | number | undefined) => {
...
const ProjectsList = React.memo(() => {
  const allProjects = useMemo(() => ([...getProjects()] as IProject[]).reverse(), []);
  ...
  useIntersectionObserver(
    ...
    () => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        setDisplayCount((prev: number) => Math.min(prev + 6, filteredProjects.length));
      });
    },
