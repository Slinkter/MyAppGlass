...
export function calcularPrecio(config: ConfiguracionCotizacion): number {
    ...
    const glassCost =
        area * (PRICING.glass[tipoVidrio as keyof typeof PRICING.glass] || 160);
    const alumCost =
        perimeter *
        45 *
        sys.multiplier *
    ...
    const hwCost =
        PRICING.baseHardware[tipoId as keyof typeof PRICING.baseHardware] || 50;
